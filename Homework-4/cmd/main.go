...
import (
	...
	"homework-4/internal/service"
	"homework-4/internal/storage"
	"time"
)
...
func main() {
	...
	flagTypeOfPackaging := flag.String("pack", "Film", "type of packaging")
	flagPrice := flag.Float64("price", 0, "price")
	flagWeight := flag.Float64("weight", 0, "weight")
	...
	case "help":
		...
		fmt.Println("  -command=accept    -id=0 -idpvz=0 -datestorage=MM-DD-YYYY -idclient=0  - Принять заказ от курьера") //-command=accept -id=0 -idpvz=0 -datestorage=00-00-0000 -idclient=0 -price=0 -weight=0 -pack=Film
		fmt.Println("                     -price=0 -weight=0 -pack=Film   (pack: Film, Package, Box)")
	...
	case "accept":
		...
		err = serv.Accept(model.DataInputAccept{
			...
			Price:           *flagPrice,
			Weight:          *flagWeight,
			TypeOfPackaging: model.PackageType(*flagTypeOfPackaging),
		})
		if err != nil {
			fmt.Println(err)
			return
