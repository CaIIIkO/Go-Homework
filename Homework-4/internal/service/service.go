...
func (s Service) Accept(input model.DataInputAccept) error {
	...
	pack, err := model.GetTypePackage(input.TypeOfPackaging)
	if err != nil {
		return err
	}

	if !pack.Validate(input.Weight) {
		return errors.New("ошибка! не подходящий вес для упоковки")
	}
	input.Price += pack.Price()
